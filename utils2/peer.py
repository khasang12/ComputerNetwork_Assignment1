    ...
    def run(self):
        while(True):
            ...
            if(choose == "1" ):
                ...
                # [ip,port] = input("Connect to IP Port: ").strip().split(" ")
                # print(f"Connected to peer {ip}, port {port}.")
                # conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                # conn.connect((ip, int(port)))
                # self.HandleConnection.createClientThread(conn)

                # print(f"Connected to peer {self.peerIp}, port {self.peerPort}.")
                # conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                # conn.connect((self.peerIp, int(self.peerPort)))
                # self.HandleConnection.createClientThread(conn)
            # else:
            #     pass
...
if __name__ == "__main__":
    ...
    ip = "127.0.0.1"        # already connected
    port = 1
    Peer(ip, port).run()